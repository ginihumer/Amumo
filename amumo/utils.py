import amumo.model as amumo_model
...
def get_embedding(model_name, dataset_name, all_images, all_prompts, batch_size=500):
    ...
    if type(model_name) == str:
        clip_model = amumo_model.get_model(model_name, device=device)
    elif issubclass(type(model_name), amumo_model.CLIPModelInterface):
    ...
    if not os.path.exists(data_checkpoint_dir + data_prefix + '_image-embedding.csv') or not os.path.exists(
            data_checkpoint_dir + data_prefix + '_text-embedding.csv'):
        # with torch.no_grad():
        ...
        image_features = torch.from_numpy(
            np.genfromtxt(data_checkpoint_dir + data_prefix + '_image-embedding.csv', delimiter=","))
        text_features = torch.from_numpy(
            np.genfromtxt(data_checkpoint_dir + data_prefix + '_text-embedding.csv', delimiter=","))

    return image_features / image_features.norm(dim=-1, keepdim=True), text_features / text_features.norm(dim=-1,
                                                                                                          keepdim=True), clip_model.logit_scale
...
def aggregate_texts(emb_ids, all_prompts, input_type=None):
    ...
    ngrams_sorted = sorted([(count_values[i], k) for k, i in vocab.items()], reverse=False,
                           key=lambda sl: (len(sl[1]), sl[0]))
