...
from .utils import get_textual_label_for_cluster, get_embedding, get_similarity, get_cluster_sorting, \
    get_modality_distance, calculate_val_loss, get_closed_modality_gap, get_modality_gap_normed, l2_norm, \
    get_gap_direction
...
class SimilarityHeatmapWidget(widgets.VBox):
    ...
    def _get_matrix_gridlines(self):
        return [
            go.layout.Shape(type='line', x0=len(self.value) / 2 - 0.5, y0=0 - 0.5, x1=len(self.value) / 2 - 0.5,
                            y1=len(self.value) - 0.5, line=dict(color="black", width=1)),
            go.layout.Shape(type='line', y0=len(self.value) / 2 - 0.5, x0=0 - 0.5, y1=len(self.value) / 2 - 0.5,
                            x1=len(self.value) - 0.5, line=dict(color="black", width=1))
        ]
        ...
        for (cluster_label, cluster_size) in zip(labels, sizes):
            if cluster_size > 5:
                ...
                cluster_shapes += [go.layout.Shape(
                    ...
                    label=dict(text=cluster_label, textposition=textposition, font=dict(size=10, color="white"),
                               padding=np.log(cluster_size) * 10),
    ...
    def onUpdateHoverIdx(self, change):
        ...
        for (x_idx, y_idx) in self.hover_idx:
            if x_idx >= 0 and x_idx < len(self.value):
                shapes.append(go.layout.Shape(name='hover_idx', type='line', x0=x_idx, y0=0 - 0.5, x1=x_idx,
                                              y1=len(self.value) - 0.5, line=dict(color="grey", width=1)))
            if y_idx >= 0 and y_idx < len(self.value):
                shapes.append(go.layout.Shape(name='hover_idx', type='line', y0=y_idx, x0=0 - 0.5, y1=y_idx,
                                              x1=len(self.value) - 0.5, line=dict(color="grey", width=1)))
...
class ScatterPlotWidget(widgets.VBox):
    ...
    def __init__(self, seed=31415, modality1_label='Image', modality2_label='Text'):
        ...
        self.fig_widget.add_trace(
            go.Scatter(name=modality1_label, x=[0, 1, 2, 3], y=[0, 1, 2, 3], mode="markers", marker_color='blue'))
        self.fig_widget.add_trace(
            go.Scatter(name=modality2_label, x=[3, 2, 1, 0], y=[0, 1, 2, 3], mode="markers", marker_color='orange'))
        ...
        self.children = [self.select_projection_method, self.use_oos_projection, self.nr_components_widget,
                         widgets.HBox([self.x_component_widget, self.y_component_widget]), self.fig_widget]
    ...
    def onUpdateValue(self, change):
        ...
        if self.select_projection_method.value == 'PCA':
            projection = projection_method['module'](n_components=self.nr_components_widget.value,
                                                     random_state=self.seed)
        else:
            projection = projection_method['module'](n_components=self.nr_components_widget.value, metric="cosine",
                                                     random_state=self.seed)
        ...
        if not self.use_oos_projection.disabled and self.use_oos_projection.value:
            ...
            if project_by == "image":
                self.image_embedding_projection = projection.fit_transform(
                    self.embedding[:int(len(self.embedding) / 2), :])
                self.text_embedding_projection = projection.transform(self.embedding[int(len(self.embedding) / 2):, :])
            elif project_by == "text":
                self.text_embedding_projection = projection.fit_transform(
                    self.embedding[int(len(self.embedding) / 2):, :])
                self.image_embedding_projection = projection.transform(self.embedding[:int(len(self.embedding) / 2), :])

            self.embedding_projection = np.concatenate(
                (self.image_embedding_projection, self.text_embedding_projection))
    ...
    def update_scatter(self, change):
        ...
        for line_idx in range(len(self.text_embedding_projection)):
            lines.append(go.layout.Shape(name='hover_idx',
                                         type='line',
                                         x0=self.image_embedding_projection[
                                             line_idx, self.x_component_widget.value - 1],
                                         y0=self.image_embedding_projection[
                                             line_idx, self.y_component_widget.value - 1],
...
class SimilarityHeatmapClusteringWidget(widgets.VBox):
    ...
    def __init__(self, zmin=None, zmax=None, cluster_label_data=None, modality1_label='Image', modality2_label='Text',
                 hover_callback=None):
    ...
    def onUpdateEmbedding(self, change):
        ...
        similarity_cross_modal, similarity_all = get_similarity(torch.from_numpy(modality1_embedding),
                                                                torch.from_numpy(modality2_embedding))
        ...
        if self.cluster_similarity_matrix_widget.value:
            ...
            self.idcs = np.arange(
                self.size)  # TODO: use reverse idcs to get original order for interaction with other widgets

        # with heatmap_widget.batch_update():
        matrix_sort_idcs = np.concatenate([self.idcs, self.idcs + self.size],
                                          axis=0)  # need to do double index because we combined images and texts
...
class CLIPExplorerWidget(widgets.AppLayout):
    ...
    def __init__(self, dataset_name, all_images, all_prompts, models=None):
        ...
        image_embedding_norm, text_embedding_norm, logit_scale = get_embedding(m, self.dataset_name, self.all_images,
                                                                               self.all_prompts)
        ...
        self.heatmap_widget = SimilarityHeatmapClusteringWidget(cluster_label_data=all_prompts,
        ...
        self.header = widgets.VBox(
            [widgets.HBox([self.model_select_widget, self.close_modality_gap_widget]), self.log_widget])
    ...
    def model_changed(self, change):
        ...
        m = self.models[self.model_select_widget.value]
        image_embedding, text_embedding, logit_scale = get_embedding(m, self.dataset_name, self.all_images,
                                                                     self.all_prompts)
...
class CLIPExplorerWidget_Old(widgets.AppLayout):
    ...
    def __init__(self, dataset_name, all_images, all_prompts, models=None):
        ...
        # display everyting
        self.header = widgets.VBox([widgets.HBox(
            [self.model_select_widget, self.cluster_similarity_matrix_widget, self.close_modality_gap_widget]),
                                    self.log_widget])
        self.header.layout.height = '80px'
        vis_widgets = widgets.HBox([self.heatmap_widget, self.scatter_widget])
        self.center = vis_widgets
...
class CLIPComparerWidget(widgets.AppLayout):
    ...
    def __init__(self, dataset_name, all_images, all_prompts, models=list(model.available_CLIP_models),
                 close_modality_gap=False, zmin=None, zmax=None):
        ...
        assert type(close_modality_gap) == list and len(close_modality_gap) == len(
            self.models), 'close_modality_gap must be a bool or list of the same length as models'
        ...
        for i in range(len(self.models.keys())):
            ...
            image_embedding_norm, text_embedding_norm, logit_scale = get_embedding(m, self.dataset_name,
                                                                                   self.all_images, self.all_prompts)
            if close_modality_gap[i]:
                image_embedding_norm, text_embedding_norm = get_closed_modality_gap(image_embedding_norm,
                                                                                    text_embedding_norm)
            ...
            text_widget = widgets.HTML(value='<h2>' + m.model_name + '</h2>'
                                             + ' Modality distance: %.2f | Loss: %.2f' % (
                                             modality_distance, validation_loss))
...
class ModalityGapWidget(widgets.AppLayout):
    ...
    def __init__(self, image_embedding, text_embedding, logit_scale=100.0, title='Loss Landscape CLIP'):
        ...
        for delta in np.arange(-5.0, 5.0, 0.25):
            ...
            avg_val_loss = calculate_val_loss(torch.from_numpy(modified_image_features),
                                              torch.from_numpy(modified_text_features), logit_scale=logit_scale)
        ...
        fig = go.FigureWidget(data=go.Scatter(x=distance_lst, y=loss_lst, mode='lines+markers',
